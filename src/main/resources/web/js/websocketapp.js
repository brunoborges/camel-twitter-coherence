...
$(window).keydown(function(event) {
	if (event.which == 27) {
		// the following seems to fix the symptom but only in case the document
		// has the focus
...
function setupGallery() {
	...
	while (localRows < rows) {
		var localCols = 0;
		$("#pictures").append(
				"<div class='imageRow' id='imageRow_" + localRows + "'>");
		while (localCols < cols) {
			$("#imageRow_" + localRows).append(
					"<div id='" + (picIndex) + "' class='single'>");
			$("#" + picIndex++).append(
					"<a href='#'><img src='images/javaone.png'/></a>");
			localCols++;
		}
		localRows++;
	}
